// Package cli builds the pypi command tree on top of the pypi library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: pypi.DefaultConfig()}

	root := &cobra.Command{
		Use:   "pypi",
		Short: "Browse the PyPI Python package index",
		Long: `pypi reads the Python Package Index through its public JSON API, HTML search,
and RSS feeds. No API key is required. Records are returned as table, JSON,
JSONL, CSV, TSV, or URLs.

pypi is an independent tool and is not affiliated with the Python Software
Foundation or PyPI.`,
		SilenceUsage:  true,
		SilenceErrors: true,
