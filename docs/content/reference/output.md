...
describes what users get. Pick a format with `-o`, or let pypi choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
pypi <command> -o table   # aligned columns for reading
pypi <command> -o jsonl   # one JSON object per line, for piping
pypi <command> -o json    # a single JSON array
pypi <command> -o csv     # spreadsheet friendly
pypi <command> -o tsv     # tab-separated
pypi <command> -o url     # just the URL column
pypi <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
pypi <command> --fields id,title,url
```
...
```bash
pypi <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
pypi <command>            # a table, because this is a terminal
pypi <command> | wc -l    # JSONL, because this is a pipe
```
